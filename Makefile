...
build-amd64:
	...
	docker --context default buildx build -f Dockerfile.amd64 --platform linux/amd64 . -t nassos/slm:$(BUILD_TAG)-amd64 --load

build-arm64:
	if docker buildx ls | grep -q slm; then echo "slm buildx already exists."; else docker buildx create --name slm --use; fi
	docker --context default buildx build -f Dockerfile.arm64 --platform linux/arm64 . -t nassos/slm:$(BUILD_TAG)-arm64 --load

build-armv7:
	if docker buildx ls | grep -q slm; then echo "slm buildx already exists."; else docker buildx create --name slm --use; fi
	docker --context default buildx build -f Dockerfile.armv7 --platform linux/arm/v7 . -t nassos/slm:$(BUILD_TAG)-armv7 --load
